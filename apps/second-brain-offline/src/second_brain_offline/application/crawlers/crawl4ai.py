...
class Crawl4AICrawler(BaseCrawler):
    ...
    def __call__(self, pages: list[Document]) -> list[Document]:
        try:
            ...
            return asyncio.run(self.__batch_crawl(pages))
        else:
            return loop.run_until_complete(self.__batch_crawl(pages))
        
        
    async def __batch_crawl(self, pages: list[Document]):
        ...
        logger.debug(f"Starting crawl with memory {start_mem}")
    ...
    async def __crawl_url(
        ...
        async with semaphore:
            ...
            if result.metadata:
                title = result.metadata.pop("title", "")
            else:
                title = ""
            
            child_links = [
                link["href"] for link in result.links['internal'] + result.links['external']
            ]
            
            document_id = utils.generate_random_hex()
