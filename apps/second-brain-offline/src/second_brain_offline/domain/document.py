...
class DocumentMetadata(BaseModel):
    ...
    def obfuscate(self) -> "DocumentMetadata":
        """Create a obfuscate version of metadata by changing in place

        Returns:
            DocumentMetadata: self with obfuscate version of metadata(id, url)
        """
...
class Document(BaseModel):
    id: str = Field(default_factory=lambda: utils.generate_random_hex(length=32)) # default_factory - when you want to dynamically generate default values for field.
    ...
    def write(
        self, output_dir: Path, obfuscate: bool, also_save_as_text: bool
    ) -> None:
        ...
        json_page = self.model_dump() # model_dump() is the new version of dict() - in pydantic v2
    ...
    def obfuscate(self) -> "Document":
        ...
        self.id = self.metadata.id  # changed to fake id
